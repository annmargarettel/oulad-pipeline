CREATE OR REPLACE TABLE oulad.clean.assessments AS
WITH typed AS (
...
-- Fix: Renamed table from 01_clean_assessments_quarantine to assessments_quarantine
CREATE OR REPLACE TABLE oulad.clean.assessments_quarantine AS
WITH typed AS (
  SELECT
    upper(trim(code_module)) AS code_module,
    upper(trim(code_presentation)) AS code_presentation,
...
SELECT 'row_count_matches_raw_minus_quarantine' AS check_name,
       (SELECT count(*) FROM oulad.clean.assessments) =
       (SELECT count(*) FROM oulad.raw.assessments) - (SELECT count(*) FROM oulad.clean.assessments_quarantine) AS passed
UNION ALL
SELECT 'id_assessment_not_null',   sum(CASE WHEN id_assessment IS NULL THEN 1 ELSE 0 END) = 0 FROM oulad.clean.assessments
UNION ALL
SELECT 'id_assessment_unique',     count(*) = count(DISTINCT id_assessment)                  FROM oulad.clean.assessments
UNION ALL
SELECT 'assessment_type_in_domain', sum(CASE WHEN assessment_type NOT IN ('TMA','CMA','Exam') THEN 1 ELSE 0 END) = 0 FROM oulad.clean.assessments
UNION ALL
SELECT 'weight_in_range_when_present',
       sum(CASE WHEN weight IS NOT NULL AND weight NOT BETWEEN 0 AND 100 THEN 1 ELSE 0 END) = 0 FROM oulad.clean.assessments
UNION ALL
SELECT 'date_positive_when_present',
       sum(CASE WHEN date IS NOT NULL AND date <= 0 THEN 1 ELSE 0 END) = 0 FROM oulad.clean.assessments
UNION ALL
SELECT 'null_date_only_on_exam',
       sum(CASE WHEN date IS NULL AND assessment_type <> 'Exam' THEN 1 ELSE 0 END) = 0 FROM oulad.clean.assessments
UNION ALL
SELECT 'no_literal_null_strings_remaining',
       sum(CASE WHEN cast(date AS STRING) = 'null' OR cast(weight AS STRING) = 'null' THEN 1 ELSE 0 END) = 0 FROM oulad.clean.assessments
UNION ALL
SELECT 'code_presentation_format',
       sum(CASE WHEN code_presentation NOT RLIKE '^[0-9]{4}[BJ]$' THEN 1 ELSE 0 END) = 0 FROM oulad.clean.assessments;
...
SELECT * FROM oulad.clean.assessments WHERE date IS NULL;

CREATE OR REPLACE TEMPORARY VIEW vw_silver_null_date_unexpected AS
SELECT * FROM oulad.clean.assessments WHERE date IS NULL AND assessment_type <> 'Exam';

CREATE OR REPLACE TEMPORARY VIEW vw_silver_null_weight AS
SELECT * FROM oulad.clean.assessments WHERE weight IS NULL;

CREATE OR REPLACE TEMPORARY VIEW vw_silver_any_null AS
SELECT * FROM oulad.clean.assessments WHERE date IS NULL OR weight IS NULL;
