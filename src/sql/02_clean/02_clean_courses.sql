...
SELECT
    ...
    TRY_CAST(TRIM(UPPER(code_module)) AS STRING) AS code_module,
    
    -- Trim whitespace, upper-case, and cast presentation code to string
    TRY_CAST(TRIM(UPPER(code_presentation)) AS STRING) AS code_presentation,

    -- Standardize module duration in days as an explicit 64-bit integer
    TRY_CAST(module_presentation_length AS BIGINT) AS module_presentation_length
